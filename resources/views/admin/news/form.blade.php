...
@section('content')
    <form id="form-data" action="{{isset($data)?route('dashboard.'.$model->route_key.'.update', $data):route('dashboard.'.$model->route_key.'.store')}}" class="form d-flex flex-column flex-lg-row">
        ...
        <div class="d-flex flex-column gap-7 gap-lg-10 w-100 w-lg-300px min-w-lg-300px mb-7 me-lg-10">
            <x-inputs.image label="{{__('dash.image')}}" name="image" required="" class="bg-dark" data="{{isset($data)?$data->image:''}}"/>
        ...
        <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
            <div class="card card-flush py-4">
                ...
                <div class="card-body py-0">
                    <x-html.div_loader />
                    <div class="row">
                        {{-- @php dd(\App\Models\Member::all()) @endphp --}}
                        <x-inputs.text label="{{__('dash.title')}}" name="title" required="" data="{{isset($data)?$data->title:''}}"/>
                            <x-inputs.select label="{{__('dash.member_id')}}" name="member_id" required="" data="{{isset($data)?$data->member_id:''}}"
                                :list="\App\Models\Member::all()" optionValue="id" optionName="name" />
                            <x-inputs.select label="{{__('dash.article_type_id')}}" name="article_type_id" required="" data="{{isset($data)?$data->article_type_id:''}}"
                                :list="\App\Models\ArticleType::all()" optionValue="id" optionName="title" />
                        <x-inputs.editor label="{{__('dash.description')}}" name="description" required="" data="{{isset($data)?$data->description:''}}"/>
                        <x-inputs.file label="{{__('dash.file one')}}" name="file_one" file="{{isset($data)?$data->file_one:''}}" fileName="{{isset($data)?$data->file_one_name:''}}" key="{{isset($data)?$data->id:''}}" data="{{'file_one'}}" accept="{{acceptFileType()}}"/>
                        <x-inputs.file label="{{__('dash.file two')}}" name="file_two" file="{{isset($data)?$data->file_two:''}}" fileName="{{isset($data)?$data->file_two_name:''}}" key="{{isset($data)?$data->id:''}}"  data="{{'file_two'}}" accept="{{acceptFileType()}}"/>
                    </div>
                </div>
            </div>
