        ...
        <div class="card-body pt-0">
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="datatable">
                ...
                @foreach($data as $record)
                    <tr>
                        <td><div class="symbol symbol-50px bg-dark p-3"><img src="{{asset($record->icone)}}" alt="" class="object-cover"></div></td>
                        <td>{{$record->name}}</td>
                        <td>{{$record->created_at}}</td>
                        @php $actionComponentName = 'action-btn.'. 'type-show' @endphp
