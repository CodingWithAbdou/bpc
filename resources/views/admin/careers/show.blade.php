        ...
        <div class="card-body pt-0">
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="datatable">
                ...
                <tr class="text-start text-dark fw-bolder fs-7 text-uppercase gs-0">
                    <th class="min-w-100px">{{__('dash.image')}}</th>
                    <th class="min-w-100px">{{__('dash.Job name')}}</th>
                    <th class="min-w-150px ">{{__('dash.name')}}</th>
                    <th class="min-w-150px ">{{__('dash.mobile')}}</th>
                    <th class="min-w-150px ">{{__('dash.email')}}</th>
                ...
                @foreach($data as $record)
                    <tr>
                        <td><div class="symbol symbol-50px bg-dark p-3"><img src="{{asset($record->image)}}" alt="" class="object-cover"></div></td>
                        <td>{{$record->position}}</td>
                        <td>{{$record->full_name}}</td>
                        <td>{{$record->mobile}}</td>
                        <td>{{$record->email}}</td>
                        <td>{{$record->created_at}}</td>
                        @php $actionComponentName = 'action-btn.careers' @endphp
