        ...
        <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
            <div class="card card-flush py-4">
                ...
                <div class="card-body py-0">
                    <x-html.div_loader />
                    <div class="row">
                        <x-inputs.text label="{{__('dash.name of career')}}" name="title" required="" data="{{isset($data)?$data->title:''}}"/>
                        <x-inputs.textarea class="" label="{{__('dash.description of career')}}" name="description" required="" data="{!! isset($data)?$data->description:'' !!}"/>
                        <x-inputs.file label="{{__('dash.file')}}" name="file" file="{{isset($data)?$data->file:''}}" fileName="{{isset($data)?$data->file_name:''}}" key="{{isset($data)?$data->id:''}}" data="{{'file_one'}}" accept="{{acceptFileType()}}"/>
