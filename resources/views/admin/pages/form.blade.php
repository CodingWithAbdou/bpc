...
@section('content')
    <form id="form-data" action="{{isset($data)?route('dashboard.'.$model->route_key.'.update', $data):route('dashboard.'.$model->route_key.'.store')}}" class="form d-flex flex-column flex-lg-row">
        ...
        <div class="d-flex flex-column gap-7 gap-lg-10 w-100 w-lg-300px min-w-lg-300px mb-7 me-lg-10">
            @if($data->section_key != 'products' and $data->section_key != 'productivity' and   $data->page_key != 'other')
                <x-inputs.image label="{{__('dash.image')}}" name="image_one" required="" class="bg-dark" data="{{isset($data)?$data->image_one:''}}"/>
            @endif
            @if($data->section_key == 'experience' || $data->section_key == 'history')
                <x-inputs.image label="{{__('dash.image')}}" name="image_two" required="" class="bg-dark" data="{{isset($data)?$data->image_two:''}}"/>
            @endif
            ...
            <div class="card card-flush py-4">
                ...
                <div class="card-body py-0">
                    ...
                    <div class="row">
                        ...
                        @if($data->section_key != 'productivity')
