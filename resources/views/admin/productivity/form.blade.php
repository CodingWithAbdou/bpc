...
@section('content')
    <form id="form-data" action="{{isset($data)?route('dashboard.'.$model->route_key.'.update', $data):route('dashboard.'.$model->route_key.'.store')}}" class="form d-flex flex-column flex-lg-row">
        ...
        <div class="d-flex flex-column gap-7 gap-lg-10 w-100 w-lg-300px min-w-lg-300px mb-7 me-lg-10">
            <x-inputs.image label="{{__('dash.image of face')}}" name="icone" required="" class="bg-dark" data="{{isset($data)?$data->icone:''}}"/>
        ...
        <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
            <div class="card card-flush py-4">
                ...
                <div class="card-body py-0">
                    <x-html.div_loader />
                    <div class="row">
                        <x-inputs.text label="{{__('dash.name of property')}}" name="name" required="" data="{{isset($data)?$data->name:''}}"/>
                        <x-inputs.number label="{{__('dash.value of property')}}" name="value" required="" data="{{isset($data)?$data->value:''}}"/>
                    </div>
                </div>
            </div>
