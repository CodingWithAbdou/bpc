...
@section('title')
    <div class="page-title d-flex flex-column me-3">
        ...
        <ul class="breadcrumb breadcrumb-dot fw-bold text-gray-600 fs-7 my-1">
            ...
            <li class="breadcrumb-item text-gray-600">
                <a href="{{isset($id)?route('dashboard.'.$model->route_key.'.index',$id):route('dashboard.'.$model->route_key.'.index')}}" class="text-gray-600 text-hover-primary">{{$model->{'title_'.app()->getLocale()} }}</a>
            </li>
...
@section('content')
    <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
                ...
                <div class="row">
                    ...
                    @if(!empty($list))
                        @foreach($list as $item)
                            <!--begin::Card-->
                                <div data-sectionid="{{$item->id}}" class="card card-custom mb-2 draggable slider-order-div">
                                    <div class="card-header draggable-handle border">
                                        <div class="card-title">
                                            <h3 class="card-label">{{$item->title_color ? $item->title . $item->title_color :  $item->name ?? strip_tags($item->title) ?? $item->file_name ?? (getLocale() == 'ar' ? $item->name_ar : $item->name_en) ?? strip_tags($item->description) }}</h3>
                                        </div>
        ...
        <div class="d-flex justify-content-end">
            <a href="{{route('dashboard.'.$model->route_key.'.index')}}" id="kt_ecommerce_add_product_cancel" class="btn btn-light me-5">Cancel</a>
        </div>
    </div>
@endsection
