...
@section('content')
    <form id="form-data" action="{{route('dashboard.environment.lists.update' , $data)}}" class="form d-flex flex-column flex-lg-row">
        ...
        <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
            <div class="card card-flush py-4">
                ...
                <div class="card-body py-0">
                    <x-html.div_loader />
                    <div class="row">
                        <x-inputs.text label="{{__('dash.title of list')}}" name="title" required="" data="{{isset($data)?$data->title:''}}"/>
                        <x-inputs.text label="{{__('dash.color title of list')}}" name="title_color" required="" data="{{isset($data)?$data->title_color:''}}"/>
                        <x-inputs.textarea class="editor" label="{{__('dash.description')}}" name="description" required="" data="{!! $data->description !!}"/>
                    </div>
                </div>
            </div>
