            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="datatable">
                ...
                @foreach($data as $record)
                    <tr>
                        <td>
                            @if(($record->image))
                            <div class="symbol symbol-50px bg-dark p-3"><img src="{{asset($record->image)}}" alt="" class="object-cover"></div></td>
                        @endif
                        <td>{{$record->title . " " . $record->title_color  }}</td>
                        <td>{{$record->created_at}}</td>
                        <x-action-btn.branches :record="$record"/>
                    </tr>
                @endforeach
                </tbody>
            </table>
